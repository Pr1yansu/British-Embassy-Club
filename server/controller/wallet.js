...
exports.addTransaction = async (req, res) => {
  try {
    ...
    if (!memberId) return res.status(400).json({ statusCode: 400, message: "Member ID is required", data: null });
    if (type !== "issue" && type !== "receive") return res.status(400).json({ statusCode: 400, message: "Invalid transaction type", data: null });
    if (isNaN(payableAmount) || isNaN(couponAmount)) return res.status(400).json({ statusCode: 400, message: "Amount should be a number", data: null });
    if (payableAmount < 0 || couponAmount < 0) return res.status(400).json({ statusCode: 400, message: "Amount should be greater than 0", data: null });
    ...
    await wallet.save();
    return res.status(201).json({ statusCode: 201, message: "Transaction added successfully", data: transaction, exception: null });

  } catch (error) {
    console.log(error);
    return res.status(500).json({ statusCode: 500, message: "Internal Server Error", exception: error, data: null });
  }
};
